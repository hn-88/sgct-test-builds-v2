                              ...
                              const vec2& size, float aspectRatio)
{
    ...
    Log::Info(fmt::format("Reading Paul Bourke spherical mirror mesh from '{}'", path.string()));
    ...
    if (std::getline(meshFile, line)) {
        ...
        if (!r) {
            throw Error(
                ...
                fmt::format("Error reading mapping type in file '{}'", path.string())
            );
        }
    }
    ...
    if (std::getline(meshFile, line)) {
        ...
        if (!r) {
            throw Error(
                ...
                fmt::format("Invalid data in file '{}'", path.string())
            );
        }
