...
Buffer generateScissMesh(const std::filesystem::path& path, BaseViewport& parent) {
    ...
    Log::Info(fmt::format("Reading SCISS mesh data from '{}'", path.string()));

    
    std::ifstream file = std::ifstream(path, std::ifstream::binary);
    if (!file.good()) {
        throw Error(2070, fmt::format("Failed to open '{}'", path.string()));
    }
    ...
    if (!file.good() || fileID[0] != 'S' || fileID[1] != 'G' || fileID[2] != 'C') {
        throw Error(2071, fmt::format("Incorrect file id in file '{}'", path.string()));
    }
    ...
    if (!file.good()) {
        throw Error(2072, fmt::format("Error parsing file version from file '{}'", path.string()));
    }
    ...
    if (!file.good()) {
        throw Error(2073, fmt::format("Error parsing type from file '{}'", path.string()));
    }
    ...
    if (!file.good()) {
        throw Error(2074, fmt::format("Error parsing view data from file '{}'", path.string()));
    }
    ...
    if (!file.good()) {
        throw Error(2075, fmt::format("Error parsing file '{}'", path.string()));
    }
    ...
    if (!file.good()) {
        throw Error(2076, fmt::format("Error parsing vertices from file '{}'", path.string()));
    }
    ...
    if (!file.good()) {
        throw Error(2077, fmt::format("Error parsing indices from file '{}'", path.string()));
    }
    ...
    if (nIndices > 0) {
        ...
        if (!file.good()) {
            throw Error(2078, fmt::format("Error parsing faces from file '{}'", path.string()));
        }
    }
