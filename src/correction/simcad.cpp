                          ...
                          const vec2& size)
{
    ...
    Log::Info(fmt::format("Reading simcad warp data from '{}'", path.string()));
    ...
    if (xmlDoc.LoadFile(p.c_str()) != tinyxml2::XML_SUCCESS) {
        ...
        throw Error(2080, fmt::format("Error loading file {}. {} {}", path.string(), s1, s2));
    }
    ...
    if (XMLroot == nullptr) {
        throw Error(
            2081,
            fmt::format("Error reading file '{}'. Missing 'GeometryFile'", path.string())
        );
    }
    ...
    if (element == nullptr) {
        throw Error(
            2082,
            fmt::format("Error reading file '{}'. Missing 'GeometryDefinition'", path.string())
        );
    }
