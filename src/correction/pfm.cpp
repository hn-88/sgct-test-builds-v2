                                      ...
                                      const vec2& size, bool textureRenderMode)
{
    ...
    Log::Info(fmt::format("Reading 3D/stereo mesh data (in PFM image) from '{}'", path.string()));
    ...
    if (!result) {
        throw Error(
            ...
            fmt::format("Invalid header syntax in file '{}'", path.string())
        );
    }
    ...
    if (!result2) {
        throw Error(
            Error::Component::Pfm, 2052,
            fmt::format("Invalid endianness value in file '{}'", path.string())
        );
    }
    if (fileFormatHeader[0] != 'P' || fileFormatHeader[1] != 'F') {
        throw Error(
            Error::Component::Pfm, 2053,
            fmt::format("Incorrect file type in file '{}'", path.string())
        );
    }
    ...
    for (int i = 0; i < numCorrectionValues; i++) {
        ...
        if (!meshFile.good()) {
            throw Error(
                ...
                fmt::format("Error reading correction values in file '{}'", path.string())
            );
        }
    }
